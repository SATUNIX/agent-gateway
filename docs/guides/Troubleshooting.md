...
| Tool invocation denied | Tool module not in `local_tools_allowlist` or blocked by security | Update `src/config/security.yaml` and `POST /security/refresh`; ensure gateway-managed tools are used via `use_gateway_tool()`. |
