...
| ----------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Discovery Mechanism**       | ⚙️ *Functional with constraints*   | Agents are auto-discovered via `src/agents/<Name>/agent.py` if they export a valid `Agent` instance or factory under the `agent` attribute. Imports are validated during runtime. |
| **Configuration Defaults**    | ⚠️ *Partially flexible*            | Requires `defaults.upstream` and `defaults.model` defined in `agents.yaml`. Missing defaults cause silent omission of the agent.                                                  |
| **Security & Tooling**        | ⚠️ *Functional but rigid*          | Enforced allowlists/denylists in `security.yaml` and `tools.yaml`. Non-decorated tools or unlisted modules are blocked.                                                           |
| **Dependency Handling**       | ❌ *Manual only*                    | No automatic dependency resolution. Agents relying on external libraries fail on import.                                                                                          |
| **Error Transparency**        | ⚠️ *Limited insight*               | API surface exposes 502 errors without detailed cause. Limited observability for SDK-related exceptions.                                                                          |
| **Performance / Scalability** | ⚙️ *Adequate for small sets*       | Scans filesystem synchronously on refresh; lacks caching or async discovery.                                                                                                      |
| **UX & Developer Experience** | ⚠️ *Manual configuration required* | YAML-based setup and naming conventions can confuse new users. No admin UI or guided configuration.                                                                               |
...
* Model IDs are derived automatically: `src/agents/ResearchAgent/agent.py` → `default/researchagent`.
...
| --------------------------------- | ------------------------------------------------------------------------------ | -------------- |
| **Export name must be `agent`**   | The gateway loads only the `agent` attribute or an explicitly referenced path. | 🔴 Must Fix    |
| **Missing defaults**              | Agents without `defaults.upstream` and `defaults.model` in YAML are ignored.   | 🔴 Must Fix    |
| **Security allowlists**           | Modules or tools not on allowlists are blocked.                                | 🔴 Must Fix    |
| **Dependency errors**             | Missing libraries prevent import; no fallback mechanism.                       | 🔴 Must Fix    |
| **Return schema mismatch**        | Non-standard return types cause adapter errors.                                | 🔴 Must Fix    |
| **Naming/Namespace issues**       | Folder naming influences model IDs unpredictably.                              | 🟡 Nice to Fix |
| **Multiple exports**              | Allowed but suffix-based naming can cause confusion.                           | 🟡 Nice to Fix |
| **Tool registry mismatch**        | Non-decorated or unregistered tools fail invocation.                           | 🟡 Nice to Fix |
| **Agents lacking `instructions`** | Not classified as valid SDK agents.                                            | 🟡 Nice to Fix |
...
1. **Rigid structure** – Developers must conform to specific export names and YAML defaults.
2. **No dependency resolver** – External libraries must be installed manually into the runtime environment.
3. **Opaque errors** – Failures during discovery or import lack clear logs or error messages.
4. **Security overreach** – Overly strict allowlists block agents or tools unexpectedly.
5. **No manifest-based metadata** – Agents cannot self-describe dependencies or metadata.

### Conditional Modifications Required

* **Adjust variable names** (`agent` → correct export)
...
* `registry/discovery.py` – Implements `_looks_like_agent` and import scanning.
* `registry/agents.py` – Manages spec creation and registration.
* `sdk_adapter/adapter.py` – Validates agent types and return values.
* `config/agents.yaml` – Defines `namespace`, `upstream`, and `model` defaults.
* `config/security.yaml` – Governs allowlists for modules and tools.
* `config/tools.yaml` – Manages gateway-managed tool declarations.
