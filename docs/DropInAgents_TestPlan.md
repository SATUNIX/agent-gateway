...
This document captures the acceptance criteria for Step 1 of the "Drop-in SDK Enablement" roadmap. It translates the findings in `docs/CodeReview.md` into concrete fixtures, scenarios, and compatibility requirements that will drive implementation in later steps.

## Goals

1. **Guarantee drop-in workflow:** Adding `agents/<AgentName>/agent.py` that defines OpenAI Agents SDK objects must automatically expose `<AgentName>` via `/v1/models` and `/v1/chat/completions` without editing YAML.
...
| `LIFECYCLE_HOOK_AGENT` | Lifecycle hooks logging start/end/tool events plus handoffs (Start/Multiply). | `agents/proper_example.py` + hooks excerpt |
...
These fixtures are stored as reusable code snippets under `tests/fixtures/dropin_agents/__init__.py`. Acceptance tests can materialize them into the `agents/` directory as part of future steps.

## Compatibility Matrix

| Feature | Required Behavior |
| --- | --- |
| Agent discovery | File-based discovery under `agents/**` auto-registers models without YAML edits. |
...
1. **Model discovery:** Writing `HANDOFF_TRIAGE_AGENT` into `agents/TriageAgent/agent.py` and refreshing should expose the agent via `/v1/models` and `/admin/agents` without YAML changes.
...
Drop-in agents that wish to reuse HTTP/MCP/local tooling defined under `config/tools.yaml` can opt in via the helper `sdk_adapter.gateway_tools.gateway_tool(tool_name)`. The helper exposes a pre-wired `function_tool` that delegates to the gateway `ToolManager` while still running inside the OpenAI Agents SDK event loop. This shim is optional; pure SDK tools remain authoritative.

### Security Guardrails for Drop-in Modules

Operators can restrict which Python modules are executed by setting `default.dropin_module_allowlist` / `dropin_module_denylist` in `config/security.yaml`. The registry consults `SecurityManager.assert_agent_module_allowed()` for every discovered module, skipping and logging (`agent.dropin.blocked`) any entry that violates the policy. This ensures untrusted code cannot be imported without explicit approval.
